...
def write_label(set, index, mode,idx,img):
...
def make_set(set, size, start):
    ...
    os.chdir(set)
    print(start)
    img_path = r"C:/Users/Adam/Desktop/MIO-TCD-Localization/images_og/train/" + str(first_column[start]) + ".jpg"
    img = imageio.imread(img_path)
    prev_index = 100000000
    
    for idx, file_name in enumerate(first_column[start:]):
        if file_name == prev_index:
            write_label(set,index=ground_truth.iloc[start + idx,0],mode="add",idx=start+idx, img=img)
        else:
            img_path = r"C:\Users\Adam\Desktop\MIO-TCD-Localization\images_og\train" + "\\" + str(ground_truth.iloc[start + idx,0]) + ".jpg"
            print(img_path)
            img = imageio.imread(img_path)
            write_label(set,index=ground_truth.iloc[start + idx,0],mode="new",idx=start+idx, img=img)
        prev_index = file_name
        if not (img_path == r"C:\Users\Adam\Desktop\MIO-TCD-Localization\images_og\train" + "\\" + str(ground_truth.iloc[start + idx + 1,0]) + ".jpg"):
            cnt += 1
            if size == cnt:
                return idx
